...
class Field_Type_Error extends Bobson_Error {
  constructor(field_name, actual, expected) {
    const message = `Invalid ${field_name}. Expected: ${get_expected_type(expected)}, found: ${get_actual_type(actual)}`
    super(message)
  }
}
...
const assert = {
  ...
  field_type(field_type, object, field_name) {
    if (typeof object[field_name] !== field_type) {
      throw new Field_Type_Error(field_name, object[field_name], field_type)
    }
  },
  function(value) {
