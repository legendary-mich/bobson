...
class Bobson_Builder {
    ...
    try {
      ...
      if (type.base_type === 'object') {
        ...
        const schema = new Object_Schema(
          type.alias, mixin, fields, type.required_fields, defaults)
        ...
        this.fill_object_defaults(type.defaults, fields, defaults)
      }
  ...
  /**
   * @param {object} raw_defaults
   * @param {Map<string, Base_Schema>} schema_fields
   * @param {Map<string, any>} schema_defaults
   * @returns {*}
   */
  fill_object_defaults(raw_defaults, schema_fields, schema_defaults) {
    for (const [key, val] of Object.entries(raw_defaults)) {
      this.schema_stack.push(`.${key}`)
      const parser = schema_fields.get(key)
      if (!parser) {
        throw new Unknown_Key_Error(key)
      }
      if (/^\??(array|object)/.test(parser.type)) {
        throw new Configuration_Error(parser.type, 'default', key, 'Defaults are not supported for arrays and objects')
      }
      const result_container = parser.create_result_container()
      if (val === null) result_container.set_result(val)
      else {
        if (typeof val !== 'string') {
          throw new Configuration_Error(parser.type, 'default', key, 'It should come as a string')
        }
        result_container.append_result(val)
      }
      const result = parser.parse(result_container.result)
      schema_defaults.set(key, result)
      this.schema_stack.pop()
    }
  }
