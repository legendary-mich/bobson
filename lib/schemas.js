...
class Base_Schema {
  constructor(type, start_state, mixin) {
    assert.string(type)
  ...
  clone(type, mixin) { throw new Not_Implemented_Error() }
  clone_as_nullable() {
    if (this.is_nullable) return this.clone(this.type, this.mixin)
    else return this.clone('?' + this.type, this.mixin)
  }
  create_result_container() { throw new Not_Implemented_Error() }

  get parser_fn() { return this.mixin.parser_fn }
  get serializer_fn() { return this.mixin.serializer_fn }
  validate_raw(result) {}
...
class Object_Schema extends Base_Schema {
  constructor(type, mixin, fields, required_fields, defaults) {
    super(type, START_OBJECT, mixin)
    assert.instance(fields, Map)
    assert.array(required_fields)
    assert.instance(defaults, Map)
  ...
  clone(type, mixin) {
    assert.string(type)
    assert.object(mixin)
    const cloned = new Object_Schema(type, mixin, this.fields, this.required_fields, this.defaults)
    return cloned
  }
...
class Array_Like_Schema extends Base_Schema {
  ...
  constructor(type, start_state, mixin, min_length, max_length) {
    super(type, start_state, mixin)
...
class Array_Schema extends Array_Like_Schema {
  ...
  static from_string_params(type, mixin, min_length, max_length, child_schema) {
    ...
    return new Array_Schema(type, mixin, min_length, max_length, child_schema)
  }

  constructor(type, mixin, min_length, max_length, child_schema) {
    super(type, START_ARRAY, mixin, min_length, max_length)
    this.child_schema = null
    if (child_schema !== null) {
      this.set_child_schema(child_schema)
    }
    this.opening_char = '['
    this.closing_char = ']'
  }
  ...
  clone(type, mixin) {
    assert.string(type)
    assert.object(mixin)
    const cloned = new Array_Schema(type, mixin, this.min_length, this.max_length, this.child_schema)
    return cloned
  }
...
class String_Schema extends Array_Like_Schema {
  ...
  static from_string_params(type, mixin, min_length, max_length, regex) {
    min_length = Array_Like_Schema.parse_length(type, 'min_length', min_length)
    max_length = Array_Like_Schema.parse_length(type, 'max_length', max_length)
    try {
    ...
    return new String_Schema(type, mixin, min_length, max_length, regex)
  }

  constructor(type, mixin, min_length, max_length, regex = null) {
    super(type, START_STRING, mixin, min_length, max_length)
    if (regex !== null) {
  ...
  clone(type, mixin) {
    assert.string(type)
    assert.object(mixin)
    const cloned = new String_Schema(type, mixin, this.min_length, this.max_length, this.regex)
    return cloned
  }
...
class Enum_Schema extends String_Schema {
  constructor(type, mixin, enums) {
    ...
    super(type, mixin, min_length, max_length)
  ...
  clone(type, mixin) {
    assert.string(type)
    assert.object(mixin)
    const cloned = new Enum_Schema(type, mixin, Array.from(this.enums))
    return cloned
  }
...
class Integer_Schema extends String_Schema {
  ...
  static parse_integer(type, mixin, label, value) {
    if (!Integer_Schema.regex.test(value))
      throw new Configuration_Error(type, label, value)
    return mixin.parser_fn(value)
  }

  static validate_min_bound(type, mixin, min_string, min_min_string) {
    min_string = min_string === 'min' ? min_min_string : min_string
    const min = Integer_Schema.parse_integer(type, mixin, 'min', min_string)
    const min_min = Integer_Schema.parse_integer(type, mixin, 'min_min', min_min_string)
    if (mixin.comparer_fn(min, min_min) < 0)
      throw new Configuration_Error(type, 'min', min_string, 'too small')
    return min_string
  }

  static validate_max_bound(type, mixin, max_string, max_max_string) {
    max_string = max_string === 'max' ? max_max_string : max_string
    const max = Integer_Schema.parse_integer(type, mixin, 'max', max_string)
    const max_max = Integer_Schema.parse_integer(type, mixin, 'max_max', max_max_string)
    if (mixin.comparer_fn(max, max_max) > 0)
      throw new Configuration_Error(type, 'max', max_string, 'too big')
    return max_string
  }

  static Int_4(type, mixin, min, max) {
    const min_min = '-2147483648'
    const max_max = '2147483647'
    min = Integer_Schema.validate_min_bound(type, mixin, min, min_min)
    max = Integer_Schema.validate_max_bound(type, mixin, max, max_max)
    return new Integer_Schema(type, mixin, min, max)
  }

  static Int_Js(type, mixin, min, max) {
  ...
  static Int_8(type, mixin, min, max) {
  ...
  constructor(type, mixin, min, max) {
    ...
    assert.field_type('function', mixin, 'comparer_fn')
    super(type, mixin, 0, Math.max(min.length, max.length), regex)
    this.min = mixin.parser_fn(min)
    this.max = mixin.parser_fn(max)
    this.min_string = min
    this.max_string = max
  ...
  get comparer_fn() { return this.mixin.comparer_fn }

  clone(type, mixin) {
    assert.string(type)
    assert.object(mixin)
    const cloned = new Integer_Schema(type, mixin, this.min_string, this.max_string)
    return cloned
  }
...
class Decimal_Schema extends String_Schema {
  ...
  constructor(type, mixin, min, max) {
    ...
    this.min_string = min
    this.max_string = max
    if (mixin.comparer_fn(this.max, this.min) < 0)
      throw new Configuration_Error(type, 'max', max, 'max < min')
  ...
  clone(type, mixin) {
    assert.string(type)
    assert.object(mixin)
    const cloned = new Decimal_Schema(type, mixin, this.min_string, this.max_string)
    return cloned
  }
