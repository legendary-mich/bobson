...
describe('clone schemas', () => {
  it('Object_Schema', () => {
    ...
    const defaults = {}
    const base = new Object_Schema('old type', old_mixins, fields, required_fields, defaults)
    const clone = base.clone('new type', new_mixins)
    ...
    strictEq(clone.parser_fn, new_mixins.parser_fn)
    strictEq(clone.serializer_fn, new_mixins.serializer_fn)
  })

  it('Array_Schema', () => {
    const min_length = 2
    const max_length = 3
    const child_schema = new Object_Schema('old type', old_mixins, new Map(), [], {})
    const base = new Array_Schema('arr', old_mixins, min_length, max_length, child_schema)
    const clone = base.clone('new type', new_mixins)
  ...
  it('String_Schema', () => {
    ...
    const base = new String_Schema('old type', old_mixins, min_length, max_length, regex)
    const clone = base.clone('new type', new_mixins)
  ...
  it('Enum_Schema', () => {
    ...
    const base = new Enum_Schema('old type', old_mixins, enums)
    const clone = base.clone('new type', new_mixins)
  ...
  it('Integer_Schema', () => {
    const min = '2'
    const max = '3'
    const base = new Integer_Schema('old type', old_mixins, min, max)
    const clone = base.clone('new type', new_mixins)
    ...
    strictEq(clone.parser_fn, new_mixins.parser_fn)
    strictEq(clone.serializer_fn, new_mixins.serializer_fn)
    strictEq(clone.comparer_fn, new_mixins.comparer_fn)
  })

  it('Decimal_Schema', () => {
    const min = '2.22'
    const max = '3.33'
    const base = new Decimal_Schema('old type', old_mixins, min, max)
    const clone = base.clone('new type', new_mixins)
